...
> {
  ...
  props: functionless.TableProps<
    Exclude<PK, number | Symbol>,
    Exclude<SK, number | Symbol>
  >;
}

const documentClient = createClientFactory(DocumentClient);

const dynamoClient = createClientFactory(DynamoDB);
...
> {
  ...
  constructor(
    readonly props: functionless.TableProps<
      Exclude<PK, number | Symbol>,
      Exclude<SK, number | Symbol>
    >,
