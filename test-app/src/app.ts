...
import { ApiIntegration, ApiRequest } from "functionless";
...
new ApiIntegration(
  (
    request: ApiRequest<{
      pathParameters: { num: number };
      queryStringParameters: { num2: number };
    }>
  ) => {
    const x = 1;
    // const y = { result: 2, ...{ a: 1 } };
    // const y = fn({ num: request.pathParameters.num, s: "hello" });
    return { z: x + request.pathParameters.num };
    // return x + request.pathParameters.num + request.queryStringParameters.num2;
  }
).addIntegration(restApi.root.addResource("{num}"));
