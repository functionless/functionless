...
> Note: Functionless does not compile itself, at the moment, functionless like `reflect` cannot be used to transform the core functionless source. `/test`s are compiled.
...
Functionless uses [`projen`](https://projen.io/), desired state project definitions. Most configuration files in the main `Functionless` package are generated by project and should not be updated manually (`package.json`, `prettierrc.json`, `tsconfig.json`, etc).
...
The compiler mutates the javascript output by typescript. By default, `jest` caches this output. If making changes to the compiler, you'll need to either make changes to your test source to force the cache to refresh, explicitly clear the jest cache `npx jest --clearCache`, or run without the cache `npx jest --cache false`.
