...
export class ASL {
  ...
  public execute(stmt: Stmt): States {
    if (isBlockStmt(stmt)) {
      ...
      const parent = stmt.findParent(
        anyOf(isFunctionLike, isForInStmt, isForOfStmt)
      );
  ...
  public eval(
    ...
    if (isPromiseExpr(expr)) {
      ...
      if (isReferenceExpr(expr.expr)) {
        ...
        if (predicate !== undefined && isFunctionLike(predicate)) {
          try {
  ...
  public throw(node: FunctionlessNode):
    ...
    const mapOrParallelClosure = node.findParent(
      anyOf(isForOfStmt, isForInStmt, isFunctionLike)
    );
...
export namespace ASL {
  ...
  export function toJsonPath(expr: Expr): string {
    if (isArrayLiteralExpr(expr)) {
      ...
      // is a FunctionLike & parent === undefined and that Function is at the top (no parent).
