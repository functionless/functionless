...
export function compile(
  ...
  return (ctx) => {
    ...
    return (sf) => {
      ...
      /**
       * Calls the `global.wrapClosure` function
       */
        ...
        return errorBoundary(() => {
          if (node === undefined) {
                    ...
                    ts.factory.createArrayLiteralExpression(
                      signature.parameters.map((parameter, i) =>
                        newExpr("Argument", [
                          (
                            parameter
                              .declarations?.[0] as ts.ParameterDeclaration
                          )?.dotDotDotToken
            ...
            return newExpr("IfStmt", [
              ...
              // else
              ...(node.elseStatement
                ? [toExpr(node.elseStatement, scope)]
                : []),
            ]);
            ...
            return newExpr("SpreadAssignExpr", [
              toExpr(node.expression, scope),
            ]);
          } else if (ts.isSpreadElement(node)) {
            return newExpr("SpreadElementExpr", [
              toExpr(node.expression, scope),
            ]);
