...
export const synthesizePatternDocument = (
   ...
   * See table: https://github.com/functionless/functionless/issues/37#issuecomment-1066313146
   *
  ...
  const evalInOperation = (expr: BinaryExpr): PatternDocument => {
    ...
    assertValidEventReference(updateEventReference, eventDecl);
  ...
  const assertOneEventReference = (
    ...
    if (leftExpr !== undefined && rightExpr !== undefined) {
      ...
      assertValidEventReference(leftExpr, eventDecl);
      return {
      ...
      assertValidEventReference(rightExpr, eventDecl);
      return {
